...
bool nom_update_mtime(char* file) {
...
char* nom_get_file_perms(char* file) {
  char* perms = (char*)calloc(1, 9);
  struct stat fi;
  stat(file, &fi);
  char* _perms = "rwx-";
...
char* nom_fsize_to_cstr(char* file) {
  struct stat fi;
  stat(file, &fi);
  struct tm* tm = localtime(&fi.st_mtime);
  char* time = (char*)calloc(1, 200);
  memset(time, '\0', 200);
