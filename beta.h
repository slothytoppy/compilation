...
unsigned int nom_cmd_compile(Nom_cmd *cmd) {
  ...
  if (exec(cmd->items)) {
    nom_log(NOM_INFO, "compiled %s %s %s", cmd->items[0],
            cmd->items[cmd->count - 3], cmd->items[cmd->count - 1]);
    return 1;
  }
  return 0;
}
...
unsigned int compile_file(char *compiler, char *flags[], char *file,
                          char *destination, const char *extension, ...) {
...
unsigned int compile_targets(unsigned int sz, char *files[], char *destination,
                             char *compiler, Cstr extension) {
  if (files == NULL || destination == NULL || compiler == NULL ||
      extension == NULL) {
...
unsigned int compile(char *compiler, char *flags[], char *origin,
                     char *destination, char *extension, ...) {
  if (origin == NULL || destination == NULL || compiler == NULL ||
      extension == NULL) {
    fprintf(stderr, "origin, destination, compiler or extension was null\n");
    return 0;
  }
      ...
      if (strcmp(dirent->d_name, ".") != 0 &&
        ...
        if (strcmp(ext(dirent->d_name), extension) == 0) {
          ...
          if (strcmp(origin, ".") == 0 && strcmp(destination, ".") == 0) {
            ...
            if (flags == NULL) {
              char *command[] = {compiler, "-o", base(dirent->d_name),
                                 dirent->d_name, NULL};
              exec(command);
            } else {
              unsigned int i = 0;
              ...
              // command=(char**)reallocarray(command, 4+flagc+1,
              // sizeof(char*));
              if (command == NULL)
                exit(1);
              for (i = 0; i < flagc; i++)
...
unsigned int compile_dir(char *origin, char *destination, char *compiler,
                         Cstr extension) {
  if (origin == NULL || destination == NULL || compiler == NULL ||
      extension == NULL) {
    fprintf(stderr, "origin, destination, compiler or extension was null\n");
      ...
      if (strcmp(dirent->d_name, ".") != 0 &&
        ...
        if (strcmp(ext(dirent->d_name), extension) == 0) {
          ...
          if (strcmp(origin, destination) == 0) {
            ...
            nom_log(NOM_DEBUG, "finished compiling %s %s", command[3],
                    command[2]);
          } else {
            if (strcmp(origin, ".") == 0) {
            ...
            nom_log(NOM_INFO, "[source]:%s [binary]:%s", origin_path,
                    upcwd(dest_path, cwdbuff));
          }
          free(dest_path);
          free(origin_path);
          free(cwdbuff);
...
unsigned int rebuild(char *file, char *compiler) {
  ...
  if (needs_rebuild(file, bin)) {
    ...
    if (exec(command)) {
      ...
      nom_log(NOM_INFO, "compiled %s %s %s", command[0], command[2],
              command[4]);
      run(bin);
      exit(0);
    }
...
int IS_LIBRARY_MODIFIED(char *lib, char *file, char *compiler) {
  ...
  if (lib_time > file_time) {
    char *command[] = {compiler, "-ggdb", file, "-o", base(file), NULL};
    if (exec(command)) {
      nom_log(NOM_INFO, "compiled %s %s -o %s", command[0], command[2],
              command[4]);
